...
export default async function handler(req: any, res: any) {
  ...
  // Endpoint de polling da Evolution API (Anulado - arquitetura 100% reativa via Webhooks em tempo real)
  if (reqUrl.startsWith('/api/poll-messages') || reqUrl.startsWith('/poll-messages')) {
    ...
    return res.status(200).json({
      status: 'disabled',
      message: 'Polling desativado permanentemente. O FinControl opera 100% de forma reativa via Webhooks em tempo real.',
